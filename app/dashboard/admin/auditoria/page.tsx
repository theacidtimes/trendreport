...
import AdminManager from "@/app/dashboard/auditoria/AdminManager";
...
// Aba app_admin-only: gestao dos admins GLOBAIS do app (app_admins) + a trilha de
// auditoria de reports. O admin/layout deixa entrar tanto app_admin quanto admin do
// tenant; esta pagina em si so faz sentido pro app_admin (ACID), entao reforca o
// gate e manda o admin do tenant de volta pra gestao de membros.
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted text-sm">
          Admins do app e a trilha de auditoria — quem alterou o quê, quando.{" "}
              ...
              >
                ...
                <div className="flex flex-col min-w-0 flex-1">
                  <span className="text-white text-sm font-medium truncate">
                    {row.cliente ?? "—"}
                    {row.slug && (
                      <span className="text-muted font-normal"> · {row.slug}</span>
                    )}
                  </span>
