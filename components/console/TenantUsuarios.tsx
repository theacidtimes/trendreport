...
import {
  ...
  removerMembro,
  type UsuarioTenant,
} from "@/app/console/usuarios-actions";
...
export default function TenantUsuarios({
  ...
  // Resultado de criacao de conta nova (senha temporaria pra repassar)
  ...
  function onDefinirPapel(userId: string, novoRole: string) {
    ...
    startTransition(async () => {
      try {
        await definirPapel(tenantId, userId, novoRole);
        router.refresh();
      } catch (e) {
  ...
  function onRemover(userId: string, mail: string) {
    if (!confirm(`Remover ${mail} deste tenant?`)) return;
    setErro(null);
    setBusyId(userId);
    startTransition(async () => {
      try {
        await removerMembro(tenantId, userId);
        router.refresh();
      } catch (e) {
        setErro(e instanceof Error ? e.message : "Erro ao remover o usuário.");
      } finally {
        setBusyId(null);
      }
    });
  ...
  function onAdicionar(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      try {
        const r = await adicionarUsuario(tenantId, email, role);
        setEmail("");
        setRole("viewer");
        if (r.criado && r.senhaTemporaria) {
          setNovaConta({ email: r.email, senha: r.senhaTemporaria });
        }
        router.refresh();
      } catch (err) {
        setErro(
          err instanceof Error ? err.message : "Erro ao adicionar o usuário."
        );
      }
    });
  }
  ...
  return (
    <section className="flex flex-col gap-3">
      <div className="flex items-baseline justify-between gap-3">
        <h2 className="kicker text-muted-2">Usuários · {usados}</h2>
        <span
      ...
      {usuarios.length === 0 ? (
        <div className="rounded-3xl bg-surface border border-border p-8 text-center text-muted text-sm">
          Nenhum usuário. Adicione o primeiro admin para ativar o tenant.
        </div>
      ) : (
      ...
      >
        <h3 className="text-white text-sm font-medium">Adicionar usuário</h3>
        <p className="text-muted-2 text-[11px] leading-relaxed -mt-1">
          Se o e-mail já tiver conta, ele é anexado ao tenant. Se não, uma conta
          é criada e a senha temporária aparece aqui para você repassar.
        </p>
        ...
        {cheio && (
          <p className="text-amber-400 text-xs">
            Teto de seats atingido ({usados}/{seats}). Amplie o plano para
